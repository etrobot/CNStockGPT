...
def calculate_support_factor(data, history_data=None, window=50):
    """
    计算50日涨跌幅因子：寻找涨跌幅接近0%的股票
    
    参数：
        data (DataFrame): 包含['代码']的股票数据，应该是已经筛选过的前100只股票
        history_data (dict): 以股票代码为键，历史数据DataFrame为值的字典，如果为None则自动获取
        window (int): 计算窗口，默认50天
    
    返回：
        DataFrame: 包含['代码','横盘指数']的因子数据，横盘指数越接近0表示横盘特征越明显
    """
    try:
        logger.info('开始计算50日涨跌幅因子...')
        ...
        for _, row in data.iterrows():
            ...
            try:
                ...
                if not stock_data.empty and len(stock_data) >= window:
                    # 计算50日涨跌幅
                    close_prices = stock_data['收盘'].tail(window + 1)  # 多取一天计算涨跌幅
                    if len(close_prices) > window:
                        price_change = (close_prices.iloc[-1] / close_prices.iloc[0] - 1) * 100
                        
                        # 添加到结果DataFrame，使用涨跌幅的绝对值作为横盘指数（越小越好）
                        result_df = pd.concat([
                            result_df, 
                            pd.DataFrame({
                                '代码': [stock_code], 
                                '横盘指数': [abs(price_change)]
                            })
                        ], ignore_index=True)
                        logger.debug(f'成功计算{stock_code}的50日涨跌幅: {price_change:.2f}%')
                else:
                    logger.warning(f'股票{stock_code}的历史数据{len(stock_data)}天，不足{window}天，跳过计算')
            except Exception as e:
                logger.warning(f'计算股票{stock_code}的涨跌幅失败: {str(e)}')
        
        # 按横盘指数排序（升序，越接近0越好）
        if not result_df.empty:
            result_df = result_df.sort_values('横盘指数', ascending=True)
            
        logger.info(f'成功计算{len(result_df)}只股票的50日涨跌幅因子')
        return result_df

    except Exception as e:
        logger.error(f'计算50日涨跌幅因子失败: {str(e)}')
        raise
